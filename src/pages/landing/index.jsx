  ...
  return (
    <div className="flex flex-col justify-start items-center w-full h-full bg-[#010306] relative overflow-hidden px-[20px] sm:px-0">
      <img
        src={Logo}
        alt="logo"
        className="z-[1] mt-[50px] w-[75px] sm:w-auto"
      />
      <video
        src={Main}
        className="absolute z-[0] left-[50%] -translate-x-[50%] sm:w-[460px] -top-[20px] sm:-top-[30px]"
        autoPlay
        muted
        loop
      />
      <h2 className="DarkerGrostesqueSemibold text-[30px] sm:text-[50px] xl:text-[80px] text-[#FEF9F1] z-[1] mt-[80px] leading-3">
        One intelligent App
      </h2>
      <h2 className="DarkerGrostesqueSemibold text-[30px] sm:text-[60px] xl:text-[110px] bg-gradient-to-l from-[#FDD220] to-[#FFFFFF] bg-clip-text text-transparent z-[1] text-center">
        to manage your work and life{" "}
      </h2>
      <h3 className="DarkerGrostesqueMedium text-[14px] sm:text-[24px] text-[#FEF9F1] mt-[20px] z-[1]">
        Built with intelligence & empathy
      </h3>
      <div className="max-w-[500px]">
        <div className="bg-transparent w-full p-[1px] rounded-[10px] mt-[40px] z-[1] animation relative">
          <div className="w-full bg-[#000] rounded-[10px] z-[5] flex gap-[10px] relative p-[10px]">
            <input
              type="email"
              className="text-[#FEF9F1] w-full text-[14px] sm:text-[18px] font-normal placeholder:text-[#FEF9F1] placeholder:text-[14px] sm:placeholder:text-[16px] bg-transparent focus:outline-none"
              value={email}
              placeholder="Enter your work email"
              onChange={handleEmail}
            />
            <button
              className={`px-[15px] py-[10px] bg-[#FDD320] text-nowrap rounded-[5px] text-[#0E1014] text-[12px] sm:text-[14px] font-medium ${
                btnLoad ? "opacity-50" : "opacity-100"
              } ${email ? "opacity-100" : "opacity-50"}`}
              onClick={handleSubmit}
              disabled={!email || emailError}
            >
              {btnLoad ? "Loading..." : "Request early access"}
            </button>
          </div>
        </div>
        {emailError && (
          <p className="text-[#FF6157] text-[14px] text-start mt-[10px] font-medium">
            {emailError}
          </p>
        )}
      </div>
      <img
        src={Glow}
        alt="Glow"
        className="absolute left-[50%] -translate-x-[50%] w-[800px] z-0 -top-[280px] xl:top-auto"
      />
      <div className="w-full overflow-hidden z-[3] radialGradient">
        <p className="font-semibold text-[14px] sm:text-[18px] lg:text-[26px] xl:text-[35px] text-[#FEF9F1] text-center mt-[50px] w-10/12 sm:w-9/12 lg:w-8/12 xl:w-1/2 mx-auto mb-[20px]">
      ...
      <div className=" p-[10px] border-[#FDD320]/[20%] h-[70px] sm:h-[90px] lg:h-[100px] xl:h-[150px] border-[1px] rounded-[20px] DarkerGrostesqueSemibold text-[24px] sm:text-[36px] lg:text-[50px] xl:text-[80px] max-w-[300px] sm:max-w-[400px] lg:max-w-[730px] w-full mx-auto relative mb-[80px] sm:mb-[120px] xl:mb-[100px]mb-[150px]">
        <div className="flex gap-[20px] absolute left-[35%] -translate-x-[50%]">
          <h3 className="text-[#FEF9F1]">It’s for</h3>
          <div className="inline-block">
            {textArray.map((text, index) => (
              <h3
                key={index}
                className={`verticalFilp absolute z-0 text-[#FFE269] ${
                  currentIndex === index ? "current-index" : "hidden"
                }`}
              >
                {text}
              </h3>
      ...
      <div className="w-9/12 mx-auto sm:flex  justify-between items-center mb-[80px]">
        ...
        <p className="text-[#D1D1D1] text-[12px] lg:text-[14px] font-normal mt-[20px] sm:mt-0">
        ...
        <div>
          <div className="flex gap-[15px] lg:gap-[20px] items-center sm:justify-end mt-[20px] sm:mt-0">
            <p className="text-[#858585] text-[14px] lg:text-[16px] font-medium">
              Contact:{" "}
            </p>
            <div className="text-center flex items-center justify-center gap-[5px] cursor-pointer">
              ...
              <p
                className="font-light text-[#FEF9F1] text-[13px] lg:text-[16px]"
