...
export default function Testimonial({ setIsVideo, setVideoIndex }) {
    ...
    return (
        <div className="flex flex-col relative items-center lg:items-start w-full lg:w-11/12 lg:ml-auto mb-[20px] lg:mb-[100px]">
            <div className='absolute bg-gradient-to-br from-[#10192C29] via-[#101B2E8F] to-[#29457B00] w-[200px] lg:w-[300px] xl:w-[600px] h-[150px] sm:h-[200px] lg:h-[300px] xl:h-[400px] rounded-l-[20px] border-[#1F44C5CC] border-l-[1px] border-t-[1px] border-b-[1px] right-0 top-[70%] lg:top-[68%] -translate-y-[50%] z-0 hidden sm:block'></div>
            ...
            <Slider {...settings} className='w-full relative testimonialSlider mt-[30px] lg:mt-[50px] px-[20px] lg:px-[0px]'>
                {Testimonials.map((item, index) => (
                    <div key={index} className='focus:outline-none relative sm:pr-[10px]' onClick={() => { setIsVideo(true); setVideoIndex(index) }}>
                        <img src={item.image} alt='profile' className='sm:w-[90%]' />
                    </div>
                ))}
            </Slider>
        </div>
